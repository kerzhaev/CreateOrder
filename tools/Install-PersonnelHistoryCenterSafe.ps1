...
function Ensure-HistoryLocalization([object]$Workbook) {
    ...
    $lastFound = $sheet.Cells.Find('*', $sheet.Cells(1, 1), -4123, 1, 1, 2, $false)
    ...
    $translations = [ordered]@{
        'history.center.title' = 'История и документы'
        'history.center.description' = 'Поиск и просмотр истории без изменения реестров.'
        'history.center.employee_query' = 'Поиск сотрудника'
        'history.center.event_id' = 'EventID'
        'history.center.document_id' = 'DocumentID'
        'history.center.read_only' = 'Только просмотр: действия выполняются после явного нажатия.'
        'history.center.search' = 'Найти'
        'history.center.open_document' = 'Открыть документ'
        'history.center.repeat_export' = 'Повторить экспорт'
        'history.center.prepare_correction' = 'Подготовить корректировку'
        'history.center.close' = 'Закрыть'
        'history.center.not_loaded' = 'Данные не загружены.'
        'history.center.action_done' = 'Действие выполнено.'
        'history.center.action_failed' = 'Ошибка действия: {error}'
    }
...
function Set-RibbonXmlInWorkbook([string]$WorkbookFilePath, [string]$XmlFilePath) {
    ...
    $tempDirectory = Join-Path ([IO.Path]::GetTempPath()) ('CreateOrderHistoryRibbon-' + [guid]::NewGuid().ToString('N'))
    ...
    if ($null -eq $rels.SelectSingleNode("//r:Relationship[@Target='customUI/customUI14.xml']", $ns)) {
        $node = $rels.CreateElement('Relationship', 'http://schemas.openxmlformats.org/package/2006/relationships'); $node.SetAttribute('Id', 'customUIRelID'); $node.SetAttribute('Type', 'http://schemas.microsoft.com/office/2007/relationships/ui/extensibility'); $node.SetAttribute('Target', 'customUI/customUI14.xml'); $null = $rels.DocumentElement.AppendChild($node); $rels.Save($relsPath)
    }
...
function Assert-HistoryRibbon([string]$WorkbookFilePath) {
    Add-Type -AssemblyName System.IO.Compression.FileSystem
    ...
    try {
        ...
        if ($xml -notmatch 'openPersonnelHistoryCenter' -or $xml -notmatch 'OnOpenPersonnelHistoryCenterClick') { throw 'Ribbon XML does not contain the personnel history center button.' }
    } finally { $archive.Dispose() }
}

if (Get-Process EXCEL, WINWORD -ErrorAction SilentlyContinue) { throw 'Excel or Word is running. Close Office before the history center import.' }
...
$formPath = Join-Path $resolvedSource 'frmPersonnelHistoryCenter.frm'
foreach ($path in @((Join-Path $resolvedSource 'mdlPersonnelHistoryCenter.bas'), (Join-Path $resolvedSource 'ModuleLocalization.bas'), (Join-Path $resolvedSource 'mdlRibbonHandlers.bas'), $formPath, (Join-Path $resolvedSource 'frmPersonnelHistoryCenter.frx'), $resolvedRibbon)) { if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Missing P4 source: $path" } }

$stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$stagingDirectory = Join-Path $projectRoot "Trash\personnel-history-center-install-probe-$stamp"
$backupDirectory = Join-Path $projectRoot "CreateOrderBackups\personnel-history-center-installed-$stamp"
...
$stagingWorkbook = Join-Path $stagingDirectory 'CreateOrder.personnel-history-center-install-probe.xlsm'
$backupWorkbook = Join-Path $backupDirectory 'CreateOrder.before-personnel-history-center-install.xlsm'
...
function Install-IntoWorkbook([string]$TargetPath, [bool]$Save) {
    ...
    try {
        ...
        Import-CodeModuleText $book 'mdlPersonnelHistoryCenter' (Join-Path $resolvedSource 'mdlPersonnelHistoryCenter.bas')
        ...
        Import-UserForm $book 'frmPersonnelHistoryCenter' $formPath
        Ensure-HistoryLocalization $book
        $probeQuery = [string]$book.Worksheets.Item('Employees').Cells(2, 1).Value2
        $report = 'NO_EMPLOYEE_FIXTURE'
        if (-not [string]::IsNullOrWhiteSpace($probeQuery)) {
            $report = [string]$excel.Run("'$($book.Name)'!mdlPersonnelHistoryCenter.BuildPersonnelHistoryCenterReport", $probeQuery)
            if ($report -notmatch 'HISTORY \| employee_id=') { throw "History center report did not run: $report" }
        }
...
Assert-HistoryRibbon $stagingWorkbook
...
if (Get-Process EXCEL, WINWORD -ErrorAction SilentlyContinue) { throw 'Office process remained after staging history center import.' }
...
Assert-HistoryRibbon $resolvedWorkbook
for ($attempt = 1; $attempt -le 120 -and (Get-Process EXCEL, WINWORD -ErrorAction SilentlyContinue); $attempt++) { Start-Sleep -Milliseconds 500 }
if (Get-Process EXCEL, WINWORD -ErrorAction SilentlyContinue) { throw 'Office process remained after working history center import.' }
Write-Output "PERSONNEL_HISTORY_CENTER_INSTALL_OK|backup=$backupWorkbook|staging=$stagingWorkbook|$workingReport"
